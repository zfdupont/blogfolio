...
to learn some more and create the stats myself.
...
To understand what VORP means here, first we have to look at BPM and how it works. 
...
just means how much a player contributes to the team while they're on the floor. 
...
Now that we have a basic understand what BPM is, let's take a look at the top rookies by BPM:
...
and having less games over the course of the season.
...
Now that we got a sense for the top contenders, let's move on to **VORP** or Value Over Replacement Player, which I've already shown you above. VORP 
represents the amount of points a player brings to the team vs. a replacement (bench) player. A random player off 
the bench is expected to perform worse than average for any of several reasons, they might not fit into the context of the team, 
they might be fresh/maladjusted, etc. VORP essentailly is BPM but cumulatively adjusted for minutes. One neat aspect of this is that 
since instead of BPM which is calibrated to the league average, VORP is relative to replacement, so it tracks fairly linearly with salary: 
a player with a VORP of 1.5 is literally 1.5x more valuable than a player with a VORP of 1.5.
...
All in all, while doing these stats compilations for rookies is subject to all kinds of inconsistencies, (rookies play less games,
get less minutes so aren't fairly represented by these stats on average), I would like to expand this to cover every team and player, 
but some adjustments to the algorithm would be likely necessary-- the coeffieicents each stat is multiplied by to calculate BPM is based
